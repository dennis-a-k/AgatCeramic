            ...
            <div class="form-group">
                ...
                <select class="form-control select2" style="width: 100%;" id="selectSizes" name="size_id">
                    ...
                    @foreach ($sizes as $size)
                        <option value="{{ $size->id }}" @selected(old('size_id', $product->size_id) == $size->id)>{{ $size->title }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" style="width: 100%;" id="selectPattern" name="pattern_id">
                    ...
                    @foreach ($patterns as $pattern)
                        <option value="{{ $pattern->id }}" @selected(old('pattern_id', $product->pattern_id) == $pattern->id)>{{ $pattern->title }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" style="width: 100%;" id="selectTexture" name="texture_id">
                    ...
                    @foreach ($textures as $texture)
                        <option value="{{ $texture->id }}" @selected(old('texture_id', $product->texture_id) == $texture->id)>{{ $texture->title }}</option>
                    @endforeach
                </select>
            </div>
