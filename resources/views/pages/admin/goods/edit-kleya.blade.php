...
@section('content')
    <form method="POST" action="{{ route('kleya.update', $product->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')

        <div class="row">
            <section class="col-md-6">
                @include('components.admin.goods.kleya.edit-kleya-information')
            </section>

            <section class="col-md-6">
                @include('components.admin.goods.kleya.edit-kleya-select')
                @include('components.admin.goods.kleya.edit-kleya-images')
            </section>
